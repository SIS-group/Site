<!DOCTYPE html>
<html>
    <head>
        <title>Assistant Registrar</title>
        <link rel="stylesheet" type="text/css" href="../css/sidepanel.css">
        <link rel="stylesheet" type="text/css" href="../css/styles.css">
        <style type="text/css">
		.sidebar a {
  			display: block;
  			color: black;
  			padding: 15%;
  			text-decoration: none;
  			text-align: center;
		}
	    </style>
    </head>

    <body>
	<div class="sidebar">
  		<a href="./assistant_registrar/studentpaymentdetails.php">Student Payment Details</a>
  		<a class="action" href=" ./assistant_registrar/studentpersonaldetails.php ">Student's Personal Details</a>
  		<a href=" ./assistant_registrar/studentresultsandgrades.php">Student Results and Grades</a>
  		<a href=" ./assistant_registrar/account_settings.php">Account settings</a>
  		<a href="../login/logout.php" style="all:unset ;padding: 25%; "><button>Log out</button></a>
	</div>


    <h2><a href = "../login/logout.php">Sign Out</a></h2>
</body>
</html>